...
Q2DI(g::Geo, level::Int) = transform_points([g.lon], [g.lat], level)[1, 1]
...
function xyz_tile(x::Int, y::Int, z::Int, cell_cube::CellCube)
    # TODO: Handle select other dims e.g. time
    geo_cube = GeoCube(cell_cube; longitudes=range(-180, 180; length=256), latitudes=range(-90, 90; length=256))

    geo_cube.data |> size |> length > 2 && @warn "Non spatial dimensions left. The first element will be selected"

    geo_cube.data.data[:, :, 1]
end
